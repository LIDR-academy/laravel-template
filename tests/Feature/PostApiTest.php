...
|
| These lock the CURRENT observable contract of GET/POST/PUT/DELETE on
| /api/posts BEFORE refactoring into the layered architecture. They must
| stay green at every step of the refactor — the Resource/Service/Request
| split may not change the response shape, status codes or auth boundaries.
|--------------------------------------------------------------------------
*/

// --- GET /api/posts (public list) ---------------------------------------

it('lists posts with relations, counts and pagination envelope', function () {
    // Arrange
    ...
    // The list view must NOT embed the comments collection.
    expect($row)->not->toHaveKey('comments');
});

it('orders posts newest-first by id', function () {
    // Arrange
...
it('filters posts by published, category, tag and title query', function () {
    // Arrange
    $tag = Tag::factory()->create();
    $category = Category::factory()->create();
    ...
    // Act + Assert — category / tag / q filters all resolve to the matching post
...
it('shows a single post with comments, authors and likes count', function () {
    // Arrange
    $author = User::factory()->create();
    $tag = Tag::factory()->create();
    $category = Category::factory()->create();
    ...
    // Assert — full contract, unwrapped (no `data` envelope)
    $response->assertOk()
        ->assertJsonStructure([
            'id', 'user_id', 'title', 'slug', 'body', 'published',
...
it('returns 404 with a message for a missing post', function () {
    // Arrange + Act
...
it('creates a post for the authenticated user with synced tags and categories', function () {
    ...
    expect(Post::find($postId)->tags->pluck('id')->sort()->values()->all())
